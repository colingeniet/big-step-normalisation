...
eval-deterministic : {Γ Δ : Con} {A : Ty} {u : Tm Δ A} {ρ : Env Γ Δ} {v v' : Val Γ A} →
                     eval u > ρ ⇒ v → eval u > ρ ⇒ v' → v ≡ v'
evals-deterministic : {Γ Δ Θ : Con} {σ : Tms Δ Θ} {ρ : Env Γ Δ} {ν ν' : Env Γ Θ} →
                      evals σ > ρ ⇒ ν → evals σ > ρ ⇒ ν' → ν ≡ ν'
$-deterministic : {Γ : Con} {A B : Ty} {f : Val Γ (A ⟶ B)} {u : Val Γ A} {v v' : Val Γ B} →
                  f $ u ⇒ v → f $ u ⇒ v' → v ≡ v'

eval-deterministic (eval[] evalσ evalu) (eval[] evalσ' evalu') =
  let evalu = tr (λ ρ → eval _ > ρ ⇒ _) (evals-deterministic evalσ evalσ') evalu in
...
eval-deterministic (evalapp evalu $u) (evalapp evalu' $u') =
  let $u = tr (λ f → f $ _ ⇒ _) (eval-deterministic evalu evalu') $u in
  $-deterministic $u $u'

evals-deterministic evalsid evalsid = refl
evals-deterministic (evals∘ evalν evalσ) (evals∘ evalν' evalσ') =
  let evalσ = tr (λ ρ → evals _ > ρ ⇒ _) (evals-deterministic evalν evalν') evalσ in
  evals-deterministic evalσ evalσ'
...
norm-deterministic (qeval evalu qu) (qeval evalu' qu') =
  let qu = tr (λ v → q v ⇒ _) (eval-deterministic evalu evalu') qu in
  q-deterministic qu qu'
