...
\emph{Big step normalisation}~(BSN) is an alternative, purely syntactic
normalisation method, proposed in~\cite{chapman2009bsn} by Chapman and the
first author for the simply-typed lambda-calculus.
...
mapped to normal forms by a function named $\q$. Normalisation $\norm$ is done
by evaluating in the identity environment, then applying $\q$ on the resulting value.
The embedding $\cul\_\cur$ is simply the inclusion of normal forms into terms.
...
\item A value $v$ of the base type is strongly computable if normalisation
  terminates on $v$.
\item A value $f$ of a function type is strongly computable if it preserves
...
\item $\q$ terminates on any strongly computable value, and conversely any
  neutral value (i.e.\ a value which is not a $\lambda$-abstraction) on which
  $\q$ terminates is strongly computable.
\item In a strongly computable environment, evaluation terminates and yields
  strongly computable results.
\end{itemize}
Termination of $\norm$ follows from those results. Completeness and
...
abstract base type $\U$, the abstract base dependent family $\El$,\footnote{%
  ...
  abstract universe, i.e.\ no element of $\U$ can be built in a closed context.
  Of course, one can still use the context to postulate the existence of type
  constructors in $\U$.
}
...
While this syntax does not contain the usual application operator of the
lambda-calculus, it can reconstructed by combining the categorical application
and the substitution of a single term $<\_>$.
\begin{alignat*}{2}
...
\section{Weakenings}
\label{sec:weakening}
In this section, we introduce a notion of weakening of contexts. For this
purpose, we use renamings, i.e.\ lists of variables. The following presentation
of variables and renamings comes from~\cite{kaposi2016normalisation}.
...
\begin{flalign*}
  ...
  \end{alignedat}
\end{flalign*}
Renamings (or weakenings), denoted by $\alpha,\beta,\gamma$, and their embedding
into substitutions are mutually defined.
\begin{flalign*}
...
The definitions of identity and composition of renamings require the appropriate
auxiliary functions. The identity uses the weakening $\wk$ of a renaming by a
type, and composition uses the application $\_[\_]$ of a renaming to a variable.
...
sets that will be defined (e.g.\ values, normal forms, \dots) have a presheaf-like
...
and predicates are sub-presheaves, meaning that they are stable under weakening.
The corresponding definitions and proofs are typically straightforward, and we
will often not mention them. We abusively denote all weakenings by $\_\ ^{+\_}$.
...
\begin{flalign*}
  ...
  \begin{alignedat}{3}
    ...
    & \_,\_ && : && (\rho : \Env\ \Gamma\ \Delta) \to \Val\ \Gamma\ (A[\cul\rho\cur]) \\
    &       &&   && \to \Env\ \Gamma\ (\Delta,A)
  \end{alignedat} &&
  \begin{alignedat}{2}
    & \cul \epsilon \cur && = \epsilon \\
...
equivalent as terms (formally, have equal embeddings), but are not equal. For
instance, if one considers a closure $\clos\ u\ \rho$ in which the body $u$
never refers to the environment $\rho$, then modifying $\rho$ yields a
distinct, yet equivalent value. Consequently, the evaluation function may map
equivalent terms to distinct values, hence can not be defined on the quotiented
syntax.
...
\begin{flalign*}
  ...
  \end{alignedat}
\end{flalign*}
Most cases are very straightforward. Note how evaluation of a lambda does
...
Proving that this definition of the evaluator as a function is correct is
...
inputs and output. For instance, we denote by $\eval\ t\ \rho \Da v$ the
proposition `$t$ evaluates to $v$ in environment $\rho$'.
\begin{flalign*}
...
neutral normal forms of the base types $\U$ and $\El$. This restriction ensures
that normal forms are sufficiently $\eta$-expanded.
\begin{flalign*}
...
step semantics of $\q$, with the following signature.
\begin{align*}
...
\end{lemma}
\begin{proof}
  The proof is the same as for lemma~\ref{lem:typeInduction}, but additionally
