...
\Cref{sec:theory} summarises the metatheory in which this paper is
...
type, and the signatures of the constructors. Inductive functions are defined
by pattern-matching. For instance:
\begin{flalign*}
...
Finally, we use the convention that free variables in definitions and lemmas are
implicitly universally quantified. When this is the case, the type can be
inferred from the context and the naming conventions.
...
Variables can be embedded into terms by applying projections to $\id$ ---
intuitively, $\id$ is the substitution formed by all the variables in context.
\begin{flalign*}
...
syntax of type theory. As further explained below, this algorithm can not yet
be formally defined as a function, and we instead define it as a relation.

We first define values and the evaluation from terms to values, then normal
forms and the function $\q$ mapping values to normal forms.
...
which forces embedding of values to be injective.
\begin{flalign*}
  & \data\ \Val\ \where \\
...
Finally, let us introduce two useful constructions on environments.
...
generalised induction principle of \cref{lem:typeInduction2} is used.
\begin{flalign*}
