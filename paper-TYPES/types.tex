...
It is an intrinsically typed syntax, with De Bruijn indices, and explicit
...
the constructors. However, because we only consider a minimalist type theory,
it is solely an abstract universe, meaning that no element of $\U$ can be
built in a closed context. Nonetheless, one may use the contexts to postulate
some type constructors in $\U$. The constructors are listed below, with
regular constructors on the left, and equality constructors on the right.
\begin{flalign*}
  % Contexts
  & \data\ \Con\ \where \\ & \ind
  \begin{alignedat}{2}
