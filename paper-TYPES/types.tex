...
\label{sec:theory}
For simplicity, this paper is presented in a strict, intentional Martin-Löf Type
Theory, extended with quotient inductive inductive types (QIIT,
cf.~\cite{kaposi2016type}). 
...
sense, i.e.\ a type for which uniqueness of identity proofs (UIP) holds.
It follows by Hedberg's theorem~\cite{hedberg1998coherence} that equality is
undecidable in this syntax, which is definitively not what we wanted.
...
\end{alignat*}
Note that the corresponding contexts is unnecessary, because it can be proved
that contexts form a set from the assumption that types are a family of sets.
